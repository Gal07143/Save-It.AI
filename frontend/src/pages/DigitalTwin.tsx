...
import { api, AssetTreeNode, PanelAssetExtracted } from '../services/api'
import {
  ChevronRight, ChevronDown, Network, Gauge, AlertCircle,
...
function ViewTreeNode({ node, depth = 0 }: { node: AssetTreeNode; depth?: number }) {
    ...
    <div>
      <div
        style={{
          ...
          background: node.is_critical ? 'rgba(254, 243, 199, 0.1)' : 'transparent',
        }}
      ...
      >
        ...
        <span style={{ fontWeight: node.is_critical ? 600 : 400, color: '#f8fafc' }}>{node.name}</span>
      ...
      {expanded && hasChildren && (
        <div>
          {node.children.map((child) => (
            <ViewTreeNode key={child.id} node={child} depth={depth + 1} />
          ))}
        </div>
      )}
    </div>
...
export default function DigitalTwin({ currentSite }: DigitalTwinProps) {
    ...
    <div style={{ padding: '1.5rem', height: 'calc(100vh - 4rem)' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
          <div>
            <h1 style={{ fontSize: '1.5rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#f8fafc' }}>
              <Network size={28} color="#10b981" />
              Digital Twin
            </h1>
            <p style={{ color: '#64748b', marginTop: '0.25rem' }}>
              {mode === 'view' ? 'View your electrical Single Line Diagram (SLD)' : 'Drag and drop to build your site\'s electrical hierarchy'}
            </p>
          </div>
          ...
          <div style={{
            ...
            <button
              ...
              style={{
                ...
                background: mode === 'view' ? '#10b981' : 'transparent',
            ...
            <button
              ...
              style={{
                ...
                gap: '0.5rem',
                padding: '0.625rem 1.25rem',
                background: mode === 'build' ? '#10b981' : 'transparent',
                color: mode === 'build' ? '#fff' : '#94a3b8',
                border: 'none',
                borderRadius: '8px',
                cursor: 'pointer',
                fontWeight: 600,
        ...
        <div className="card" style={{ padding: '1rem', background: '#1e293b', border: '1px solid #334155' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <div style={{ flex: 1, maxWidth: '300px' }}>
              <label style={{ display: 'block', fontSize: '0.75rem', color: '#94a3b8', marginBottom: '0.25rem' }}>Select Site</label>
              <select
                value={selectedSite || ''}
      ...
      {mode === 'view' ? (
        <ViewMode selectedSite={selectedSite} />
      ) : (
        <BuildMode currentSite={selectedSite} />
      )}
    </div>
  )
}
...
function ViewMode({ selectedSite }: { selectedSite: number | null }) {
  ...
  if (!selectedSite) {
    return (
      <div className="card" style={{ textAlign: 'center', padding: '3rem', background: '#1e293b', border: '1px solid #334155' }}>
        <Network size={48} color="#64748b" style={{ margin: '0 auto 1rem' }} />
        <h3 style={{ marginBottom: '0.5rem', color: '#f8fafc' }}>Select a Site</h3>
    ...
    <div className="card" style={{ background: '#1e293b', border: '1px solid #334155' }}>
      ...
      {treeLoading ? (
        ...
        <div style={{ padding: '1rem' }}>
          <div style={{ border: '1px solid #334155', borderRadius: '0.5rem', padding: '0.5rem', background: '#0f172a' }}>
            {tree.map((node) => (
              <ViewTreeNode key={node.id} node={node} />
            ))}
          </div>
        </div>
      ) : (
        <div style={{ textAlign: 'center', padding: '3rem', color: '#64748b' }}>
          ...
          <p style={{ color: '#94a3b8' }}>No assets found for this site.</p>
          <p style={{ fontSize: '0.875rem', marginTop: '0.5rem' }}>Switch to Build mode to create your digital twin.</p>
        </div>
      )}
    </div>
  )
...
function BuildMode({ currentSite }: { currentSite: number | null }) {
  ...
  const renderAssetTreeContent = () => (
    <>
      ...
      {showTemplates && (
        <div className="card" style={{ marginBottom: '1rem', padding: '1rem', background: '#1e293b', border: '1px solid #334155' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <h3 style={{ fontSize: '1rem', fontWeight: 600, color: '#f8fafc' }}>Quick Templates</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem' }}>
            {TEMPLATES.map((template, i) => (
              ...
              >
                <div style={{ fontWeight: 500, marginBottom: '0.25rem', color: '#f8fafc' }}>{template.name}</div>
      ...
      {!currentSite && (
        <div className="card" style={{ textAlign: 'center', padding: '3rem', marginBottom: '1rem', background: '#1e293b', border: '1px solid #334155' }}>
      ...
      <div style={{ display: 'flex', gap: '1rem', height: 'calc(100% - 160px)' }}>
        {showPalette && (
          <div className="card" style={{ width: '200px', padding: '1rem', flexShrink: 0, background: '#1e293b', border: '1px solid #334155' }}>
    ...
    <>
      ...
      {editingNode && (
        <div style={{
          ...
          <div className="card" style={{ width: '400px', padding: '1.5rem', background: '#1e293b', border: '1px solid #334155' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h3 style={{ fontSize: '1.125rem', fontWeight: 600, color: '#f8fafc' }}>Edit Asset</h3>
