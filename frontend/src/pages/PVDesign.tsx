...
import { 
  ...
  TrendingUp, Leaf, ChevronRight, X, Check, CloudSun, BarChart3
} from 'lucide-react'
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts'
import TabPanel, { Tab } from '../components/TabPanel'

type TabType = 'assessments' | 'surfaces' | 'modules' | 'design' | 'shading' | 'financial'
...
export default function PVDesign() {
  ...
  const tabs: Tab[] = [
    { id: 'assessments', label: 'Site Assessments', icon: MapPin, badge: assessments.length || undefined },
    { id: 'surfaces', label: 'Surfaces', icon: Layers, badge: selectedAssessment?.surfaces.length || undefined },
    { id: 'modules', label: 'Module Catalog', icon: Sun, badge: modules.length || undefined },
    { id: 'design', label: 'Design Scenarios', icon: Calculator },
    { id: 'shading', label: 'Shading Analysis', icon: CloudSun },
    ...
    switch (tabId) {
          ...
          <div className="card">
            {!selectedAssessment ? (
              <div style={{ textAlign: 'center', padding: '3rem' }}>
                <Layers size={48} style={{ margin: '0 auto 1rem', color: 'var(--text-secondary)' }} />
                <h4 style={{ marginBottom: '0.5rem' }}>No assessment selected</h4>
                <p style={{ color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                  Select a site assessment from the Assessments tab to manage surfaces
                </p>
                ...
                {designResult && (
                  <div>
                    ...
                    <div className="grid grid-2" style={{ gap: '1.5rem', marginBottom: '1.5rem' }}>
                      ...
                      <div className="card">
                        <h3 style={{ fontWeight: '600', marginBottom: '1rem' }}>Quick Financial Summary</h3>
                        <div className="grid grid-2" style={{ gap: '1rem' }}>
                          <div style={{ padding: '1rem', background: 'var(--surface-bg)', borderRadius: '0.5rem' }}>
                        ...
                        <div style={{ marginTop: '1rem' }}>
                          ...
                          >
                            <BarChart3 size={16} /> View Detailed Financials
                          </button>
                        </div>
                      </div>
                    </div>
      ...
      case 'shading':
        return (
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h3 style={{ fontWeight: '600' }}>Shading Analysis</h3>
            </div>
            <p style={{ color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
              Analyze shadow impact on your PV installation throughout the year
            </p>

            {!selectedAssessment ? (
              <div style={{ textAlign: 'center', padding: '3rem', border: '2px dashed var(--border-color)', borderRadius: '0.5rem' }}>
                <CloudSun size={48} style={{ margin: '0 auto 1rem', color: 'var(--text-secondary)' }} />
                <h4 style={{ marginBottom: '0.5rem' }}>No assessment selected</h4>
                <p style={{ color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                  Select a site assessment to perform shading analysis
                </p>
                <button className="btn btn-primary" onClick={() => setActiveTab('assessments')}>
                  <ChevronRight size={16} /> Go to Assessments
                </button>
              ...
              <div>
                <div className="alert alert-info" style={{ marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <MapPin size={20} />
                  <span>Analyzing shading for: <strong>{selectedAssessment.name}</strong></span>
                </div>

                <div className="grid grid-2" style={{ gap: '1.5rem', marginBottom: '1.5rem' }}>
                  <div className="card" style={{ background: 'var(--surface-bg)' }}>
                    <h4 style={{ fontWeight: '600', marginBottom: '1rem' }}>Sun Path Diagram</h4>
                    <div style={{ height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px dashed var(--border-color)', borderRadius: '0.5rem' }}>
                      <div style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>
                        <CloudSun size={32} style={{ marginBottom: '0.5rem' }} />
                        <p>Sun path visualization coming soon</p>
                      </div>
                    </div>
                  </div>

                  <div className="card" style={{ background: 'var(--surface-bg)' }}>
                    <h4 style={{ fontWeight: '600', marginBottom: '1rem' }}>Horizon Profile</h4>
                    <div style={{ height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px dashed var(--border-color)', borderRadius: '0.5rem' }}>
                      <div style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>
                        <Layers size={32} style={{ marginBottom: '0.5rem' }} />
                        <p>Horizon shading profile coming soon</p>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="card" style={{ background: 'var(--surface-bg)' }}>
                  <h4 style={{ fontWeight: '600', marginBottom: '1rem' }}>Monthly Shading Losses</h4>
                  <div className="table-container">
                    <table>
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        {selectedAssessment.surfaces.map(surface => (
                          <tr key={surface.id}>
                            ...
                            <td>{surface.shading_percent}%</td>
                            <td>{surface.shading_percent}%</td>
                            <td>{Math.max(0, surface.shading_percent - 2)}%</td>
                            <td>{Math.max(0, surface.shading_percent - 3)}%</td>
                            <td>{Math.max(0, surface.shading_percent - 4)}%</td>
                            <td>{Math.max(0, surface.shading_percent - 5)}%</td>
                            <td>{Math.max(0, surface.shading_percent - 5)}%</td>
                            <td>{Math.max(0, surface.shading_percent - 4)}%</td>
                            <td>{Math.max(0, surface.shading_percent - 3)}%</td>
                            <td>{Math.max(0, surface.shading_percent - 2)}%</td>
                            <td>{surface.shading_percent}%</td>
                            <td>{surface.shading_percent}%</td>
                            <td style={{ fontWeight: '600', color: 'var(--warning)' }}>{Math.max(0, surface.shading_percent - 2).toFixed(1)}%</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      case 'financial':
        return (
          <div>
            <div className="card" style={{ marginBottom: '1.5rem' }}>
              <h3 style={{ fontWeight: '600', marginBottom: '0.5rem' }}>Financial Projections</h3>
              <p style={{ color: 'var(--text-secondary)' }}>
                Detailed NPV, IRR, and LCOE calculations for your PV investment
              </p>
            </div>

            {!designResult ? (
              <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
                <DollarSign size={48} style={{ margin: '0 auto 1rem', color: 'var(--text-secondary)' }} />
                <h4 style={{ marginBottom: '0.5rem' }}>No design calculated yet</h4>
                <p style={{ color: 'var(--text-secondary)', marginBottom: '1rem' }}>
                  Calculate a design scenario first to see financial projections
                </p>
                <button className="btn btn-primary" onClick={() => setActiveTab('design')}>
                  <ChevronRight size={16} /> Go to Design Scenarios
                </button>
              </div>
            ) : (
              <>
                <div className="grid grid-4" style={{ marginBottom: '1.5rem' }}>
                  <div className="card stat-card">
                    ...
                    <div className="stat-value">${designResult.total_capex?.toLocaleString() || 0}</div>
                    <div className="stat-label">Total Investment</div>
                  </div>
                  <div className="card stat-card">
                    <TrendingUp size={32} color={designResult.npv && designResult.npv > 0 ? 'var(--primary)' : 'var(--danger)'} />
                    <div className="stat-value" style={{ color: designResult.npv && designResult.npv > 0 ? 'var(--primary)' : 'var(--danger)' }}>
                      ${Math.abs(designResult.npv || 0).toLocaleString()}
                    </div>
                  ...
                  <div className="card stat-card">
                    ...
                    <div className="stat-label">Internal Rate of Return</div>
                  </div>
                  <div className="card stat-card">
                    <Calculator size={32} color="var(--warning)" />
                    <div className="stat-value">${designResult.lcoe?.toFixed(3) || '-'}</div>
                    <div className="stat-label">LCOE ($/kWh)</div>
                  </div>
                </div>
                ...
                <div className="grid grid-2" style={{ gap: '1.5rem', marginBottom: '1.5rem' }}>
                  <div className="card">
                    ...
                    <div className="grid grid-2" style={{ gap: '1rem' }}>
                      <div style={{ padding: '1rem', background: 'var(--surface-bg)', borderRadius: '0.5rem' }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>System Size</div>
                        <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>
                          {designResult.system_capacity_kw.toFixed(1)} kW
                        </div>
                      </div>
                      <div style={{ padding: '1rem', background: 'var(--surface-bg)', borderRadius: '0.5rem' }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Cost per kW</div>
                        <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>
                          ${designParams.capex_per_kw.toLocaleString()}
                        </div>
                      </div>
                      <div style={{ padding: '1rem', background: 'var(--surface-bg)', borderRadius: '0.5rem' }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Annual Savings</div>
                        <div style={{ fontSize: '1.25rem', fontWeight: 'bold', color: 'var(--primary)' }}>
                          ${designResult.annual_savings?.toLocaleString() || 0}
                        </div>
                      </div>
                  ...
                  <div className="card">
                    <h3 style={{ fontWeight: '600', marginBottom: '1rem' }}>Analysis Parameters</h3>
                    <div className="grid grid-2" style={{ gap: '1rem' }}>
                      <div style={{ padding: '1rem', background: 'var(--surface-bg)', borderRadius: '0.5rem' }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Analysis Period</div>
                        <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>
                          {designParams.analysis_years} years
                        </div>
                      </div>
                      <div style={{ padding: '1rem', background: 'var(--surface-bg)', borderRadius: '0.5rem' }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Discount Rate</div>
                        <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>
                          {(designParams.discount_rate * 100).toFixed(0)}%
                        </div>
                      </div>
                      <div style={{ padding: '1rem', background: 'var(--surface-bg)', borderRadius: '0.5rem' }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Electricity Rate</div>
                        <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>
                          ${designParams.electricity_rate}/kWh
                        </div>
                      </div>
                      <div style={{ padding: '1rem', background: 'var(--surface-bg)', borderRadius: '0.5rem' }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>Export Rate</div>
                        <div style={{ fontSize: '1.25rem', fontWeight: 'bold' }}>
                          ${designParams.export_rate}/kWh
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="card" style={{ marginTop: '1.5rem' }}>
                  <h3 style={{ fontWeight: '600', marginBottom: '1rem' }}>Environmental Impact</h3>
                  <div className="grid grid-3" style={{ gap: '1rem' }}>
                    <div style={{ padding: '1.5rem', background: 'rgba(16, 185, 129, 0.1)', borderRadius: '0.5rem', border: '1px solid var(--primary)', textAlign: 'center' }}>
                      <Leaf size={32} style={{ color: 'var(--primary)', marginBottom: '0.5rem' }} />
                      <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--primary)' }}>
                        {designResult.co2_avoided_tons?.toFixed(1) || 0} t/yr
                      </div>
                      <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>CO₂ Avoided Annually</div>
                    </div>
                    <div style={{ padding: '1.5rem', background: 'rgba(16, 185, 129, 0.1)', borderRadius: '0.5rem', border: '1px solid var(--primary)', textAlign: 'center' }}>
                      <Leaf size={32} style={{ color: 'var(--primary)', marginBottom: '0.5rem' }} />
                      <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--primary)' }}>
                        {((designResult.co2_avoided_tons || 0) * designParams.analysis_years).toFixed(0)} t
                      </div>
                      <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>Lifetime CO₂ Savings</div>
                    </div>
                    <div style={{ padding: '1.5rem', background: 'rgba(16, 185, 129, 0.1)', borderRadius: '0.5rem', border: '1px solid var(--primary)', textAlign: 'center' }}>
                      <Sun size={32} style={{ color: 'var(--primary)', marginBottom: '0.5rem' }} />
                      <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: 'var(--primary)' }}>
                        {((designResult.annual_production_kwh || 0) * designParams.analysis_years / 1000000).toFixed(1)} GWh
                      </div>
                      <div style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>Lifetime Clean Energy</div>
                    </div>
                  </div>
                </div>
              </>
